...
public class UserDao implements UserInterfaceDao{
	...
	public Integer userTotalRecordCount() {   
		String hqlString = "select count(*) from UserModel";   
	...
	public List<UserTO> findPaginated(Integer pageNo, Integer pageSize, UserTO searchUserTo) {
		...
		if(searchUserTo!=null) {
			...
			if(searchUserTo.getPhoneNumber()!=null) {
				if(!whereAdded) {
					hqlQuery = hqlQuery +" where phoneNumber like '"+searchUserTo.getPhoneNumber()+"'";
				}else {
					hqlQuery = hqlQuery +" and phoneNumber like '"+searchUserTo.getPhoneNumber()+"'";
				}
				whereAdded = true;
			}
